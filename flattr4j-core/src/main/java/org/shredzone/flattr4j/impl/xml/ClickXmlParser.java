...
/**
 * Parses an XML document for {@link Click} entries.
 *
 * @author Richard "Shred" Körber
 * @version $Revision$
 */
public class ClickXmlParser extends AbstractXmlParser<Click> {
    ...
    private Click current = null;
    private boolean insideThing = false;

    public ClickXmlParser(InputStream in) throws FlattrException {
        super(in);
    }
    ...
    @Override
    protected Click parseEndElement(QName tag, String body) throws FlattrException {
        Click result = null;
        ...
        if (QN_CLICK.equals(tag) && current != null) {
            ...
            try {
                current.setClickTime(new Date(Long.parseLong(body) * 1000L));
            ...
            current.setThingTitle(body);
            
        } else if (QN_URL.equals(tag) && current != null && insideThing) {
            current.setThingUrl(body);
        }
